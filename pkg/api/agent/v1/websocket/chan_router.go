...
func NewChanRouter(chans interfaces.AgentMessageChan) *ChanRouter {
	...
	router := &ChanRouter{
		...
		endOfThoughtResponse:         make([]*chan *interfaces.EndOfThoughtResponse, 0),
	...
	if chans != nil {
		...
		router.endOfThoughtResponse = append(router.endOfThoughtResponse, chans.GetEndOfThought()...)
...
func (r *ChanRouter) processEndOfThought(byMsg []byte) error {
	action := func(data []byte) error {
		var msg interfaces.EndOfThoughtResponse
		if err := json.Unmarshal(byMsg, &msg); err != nil {
			klog.V(1).Infof("json.Unmarshal(EndOfThoughtResponse) failed. Err: %v\n", err)
			return err
		}

		for _, ch := range r.endOfThoughtResponse {
			*ch <- &msg
		}
		return nil
	}

	return r.processGeneric(string(interfaces.TypeEndOfThoughtResponse), byMsg, action)
}
...
func (r *ChanRouter) Message(byMsg []byte) error {
	...
	case interfaces.TypeEndOfThoughtResponse:
		err = r.processEndOfThought(byMsg)
