...
// NewCallbackRouter creates a CallbackRouter with the provided FluxSpeakMessageCallback.
func NewCallbackRouter(callback interfaces.FluxSpeakMessageCallback) *CallbackRouter {
	var debugStr string
...
// Binary forwards binary audio frames to the callback.
func (r *CallbackRouter) Binary(byMsg []byte) error {
	return r.callback.Binary(byMsg)
}

// Message decodes the "type" field of a server JSON message and routes to the
// appropriate callback method.
func (r *CallbackRouter) Message(byMsg []byte) error {
	klog.V(6).Infof("fluxspeak.CallbackRouter.Message ENTER\n")
	...
	if err := json.Unmarshal(byMsg, &mt); err != nil {
		...
		klog.V(6).Infof("fluxspeak.CallbackRouter.Message LEAVE\n")
		return err
	}
...
func (r *CallbackRouter) UnhandledMessage(byMsg []byte) error {
	klog.V(6).Infof("fluxspeak.CallbackRouter.UnhandledMessage ENTER\n")
	r.printDebugMessages(3, "UnhandledMessage", byMsg)
	klog.V(1).Infof("Unknown Flux TTS event received\n")
	klog.V(6).Infof("fluxspeak.CallbackRouter.UnhandledMessage LEAVE\n")
...
func (r *CallbackRouter) processSpeechStarted(byMsg []byte) error {
	var msg interfaces.SpeechStartedResponse
	if err := json.Unmarshal(byMsg, &msg); err != nil {
		return err
	}
	return r.processGeneric("SpeechStartedResponse", byMsg, func(data *interface{}) error {
		return r.callback.SpeechStarted(&msg)
	}, msg)
}

func (r *CallbackRouter) processSpeechMetadata(byMsg []byte) error {
	var msg interfaces.SpeechMetadataResponse
	if err := json.Unmarshal(byMsg, &msg); err != nil {
		return err
	}
	return r.processGeneric("SpeechMetadataResponse", byMsg, func(data *interface{}) error {
		return r.callback.SpeechMetadata(&msg)
	}, msg)
}

func (r *CallbackRouter) processSpeechInterrupted(byMsg []byte) error {
	var msg interfaces.SpeechInterruptedResponse
	if err := json.Unmarshal(byMsg, &msg); err != nil {
		return err
	}
	return r.processGeneric("SpeechInterruptedResponse", byMsg, func(data *interface{}) error {
		return r.callback.SpeechInterrupted(&msg)
	}, msg)
}
...
func (r *CallbackRouter) processFlushed(byMsg []byte) error {
	...
	return r.processGeneric("FlushedResponse", byMsg, func(data *interface{}) error {
		return r.callback.Flushed(&msg)
	}, msg)
}

func (r *CallbackRouter) processSessionMetadata(byMsg []byte) error {
	var msg interfaces.SessionMetadataResponse
	if err := json.Unmarshal(byMsg, &msg); err != nil {
		return err
	}
	return r.processGeneric("SessionMetadataResponse", byMsg, func(data *interface{}) error {
		return r.callback.SessionMetadata(&msg)
	}, msg)
}
