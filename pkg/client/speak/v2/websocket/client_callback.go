...
// Package websocketv2 provides the Flux TTS (v2/speak) WebSocket client implementation.
package websocketv2
...
func (c *WSCallback) ping(ctx context.Context) {
	...
	defer func() {
		...
		klog.V(6).Infof("fluxspeak.WSCallback.ping() LEAVE\n")
	}()
	...
	for {
		select {
		case <-ctx.Done():
			klog.V(3).Infof("fluxspeak.WSCallback.ping() Exiting\n")
			return
		case <-ticker.C:
...
func (c *WSCallback) ProcessMessage(wsType int, byMsg []byte) error {
	klog.V(6).Infof("fluxspeak.WSCallback.ProcessMessage() ENTER\n")
	...
	klog.V(6).Infof("fluxspeak.WSCallback.ProcessMessage() LEAVE\n")
	return err
}
...
func (c *WSCallback) Speak(text string) error {
	klog.V(7).Infof("fluxspeak.WSCallback.Speak() ENTER\n")
	...
	if err := c.WriteJSON(msg); err != nil {
		...
		klog.V(7).Infof("fluxspeak.WSCallback.Speak() LEAVE\n")
		return err
	}
...
func (c *WSCallback) Flush() error {
	klog.V(7).Infof("fluxspeak.WSCallback.Flush() ENTER\n")
	...
	if err := c.WriteJSON(msg); err != nil {
		...
		klog.V(7).Infof("fluxspeak.WSCallback.Flush() LEAVE\n")
		return err
	}
...
func (c *WSCallback) Interrupt() error {
...
func (c *WSCallback) InterruptWithOffset(playedMs int) error {
	return c.sendInterrupt(&msginterfaces.InterruptMessage{
		Type: MessageTypeInterrupt,
...
func (c *WSCallback) sendInterrupt(msg *msginterfaces.InterruptMessage) error {
	klog.V(7).Infof("fluxspeak.WSCallback.Interrupt() ENTER\n")

	if err := c.WriteJSON(msg); err != nil {
		klog.V(1).Infof("Interrupt failed. Err: %v\n", err)
		klog.V(7).Infof("fluxspeak.WSCallback.Interrupt() LEAVE\n")
		return err
	}
...
func (c *WSCallback) Configure(opts *clientinterfaces.SpeakV2ConfigureOptions) error {
	klog.V(7).Infof("fluxspeak.WSCallback.Configure() ENTER\n")

	if opts == nil {
		klog.V(1).Infof("Configure: nil options\n")
		klog.V(7).Infof("fluxspeak.WSCallback.Configure() LEAVE\n")
...
func (c *WSCallback) Finish(ctx context.Context) error {
	klog.V(7).Infof("fluxspeak.WSCallback.Finish() ENTER\n")
	...
	if err := c.WriteJSON(msginterfaces.CloseMessage{Type: MessageTypeClose}); err != nil {
		...
		klog.V(7).Infof("fluxspeak.WSCallback.Finish() LEAVE\n")
		return err
	}
