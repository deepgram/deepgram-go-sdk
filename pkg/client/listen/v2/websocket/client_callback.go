...
// Package websocketv2 provides the Flux (v2/listen) WebSocket client implementation.
package websocketv2
...
func (c *WSCallback) ping() {
	klog.V(6).Infof("flux.WSCallback.ping() ENTER\n")
	...
	defer func() {
		...
		klog.V(6).Infof("flux.WSCallback.ping() LEAVE\n")
	}()
	...
	for {
		select {
		case <-c.ctx.Done():
			klog.V(3).Infof("flux.WSCallback.ping() Exiting\n")
			return
		case <-ticker.C:
...
func (c *WSCallback) Write(p []byte) (int, error) {
	klog.V(7).Infof("flux.WSCallback.Write() ENTER\n")

	byteLen := len(p)
	if err := c.WriteBinary(p); err != nil {
		klog.V(1).Infof("Write failed. Err: %v\n", err)
		klog.V(7).Infof("flux.WSCallback.Write() LEAVE\n")
		return 0, err
	}
...
func (c *WSCallback) Stream(r io.Reader) error {
	klog.V(6).Infof("flux.WSCallback.Stream() ENTER\n")
	...
	for {
		...
		case <-c.ctx.Done():
			...
			klog.V(6).Infof("flux.WSCallback.Stream() LEAVE\n")
			return nil
		default:
...
func (c *WSCallback) Configure(opts *clientinterfaces.FluxConfigureOptions) error {
	klog.V(7).Infof("flux.WSCallback.Configure() ENTER\n")
	...
	if err := c.WriteJSON(msg); err != nil {
		...
		klog.V(7).Infof("flux.WSCallback.Configure() LEAVE\n")
		return err
	}
