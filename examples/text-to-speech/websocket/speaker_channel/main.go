...
import (
	...
	speaker "github.com/deepgram/deepgram-go-sdk/pkg/audio/speaker"
)
...
func NewMyHandler(s *speaker.Speaker) MyHandler {
	handler := MyHandler{
	...
	go func() {
		...
		for br := range dch.binaryChan {
			...
			if time.Since(lastTime) > 3*time.Second {
				...
				_, err := dch.audioSpeaker.Write(header)
				if err != nil {
					fmt.Printf("Failed to play header on speaker. Err: %v\n", err)
					continue
				}
				lastTime = time.Now()
			}

			_, err := dch.audioSpeaker.Write(*br)
			if err != nil {
				fmt.Printf("Failed to play bytes on speaker. Err: %v\n", err)
				continue
			}
		}
	}()
...
func main() {
	...
	speak.Init(speak.InitLib{
		LogLevel: speak.LogLevelTrace, // LogLevelDefault, LogLevelFull, LogLevelDebug, LogLevelTrace
	})

	// Initialize portaudio
	speaker.Initialize()

	audioSpeaker, err := speaker.New(speaker.AudioConfig{
		OutputChannels: 1,
		SamplingRate:   48000,
	})
	if err != nil {
		fmt.Printf("Error creating speaker: %v\n", err)
		return
	}

	// Start the speaker
	err = audioSpeaker.Start()
	if err != nil {
		fmt.Printf("Error starting speaker: %v\n", err)
		return
	}
	...
	// create the callback
	callback := NewMyHandler(audioSpeaker)
...
EXIT:
	...
	// Stop the speaker
	err = audioSpeaker.Stop()
	if err != nil {
		fmt.Printf("Error stop speaker: %v\n", err)
	}
